...
class TeachTimeTableScreen extends StatefulWidget {
  const TeachTimeTableScreen({super.key});

  @override
  State<TeachTimeTableScreen> createState() => _TeachTimeTableScreenState();
}

class _TeachTimeTableScreenState extends State<TeachTimeTableScreen> {
  ...
  Stream<QuerySnapshot> getTimeStreams(){
    final timeStream = timetable.doc("teacher$teacherId").collection(today).snapshots();
    return timeStream;
  }
  ...
  void initState() {
    ...
    usersRef.doc(auth?.phoneNumber.toString()).get().then((DocumentSnapshot snapshot){
      teacherId = snapshot.get('teachid');
      setState(() {
        ...
        Column(
          children: [
            ...
            StreamBuilder<QuerySnapshot>(
              stream: timetable.doc("teacher$teacherId").collection(today).snapshots(),
